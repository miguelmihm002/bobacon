...
Name 'Bobacon 2.6.1'
OutFile 'Bobacon_2.6.1.exe'

!define MUI_ICON 'bobacon.ico'
...
Section 'Setup Your Environment'
	...
	nsExec::Exec '$LOCALAPPDATA\Programs\Miniconda3\Scripts\conda.exe create --name bobacon python=3.10 --yes'
SectionEnd
...
Section 'Create Install Batch'
	...
	FileWrite $0 '@echo off && conda activate bobacon && conda install conda-forge::ffmpeg=7.0.1 --yes'
	${If} $UseCuda == 1
		FileWrite $1 '@echo off && conda activate bobacon && conda install cudatoolkit=11.8 cudnn=8.9.2.26 conda-forge::gputil=1.4.0 conda-forge::zlib-wapi --yes'
		FileWrite $2 '@echo off && conda activate bobacon && python install.py --onnxruntime cuda-11.8'
	${ElseIf} $UseDirectMl == 1
		FileWrite $2 '@echo off && conda activate bobacon && python install.py --onnxruntime directml'
	${ElseIf} $UseOpenVino == 1
		FileWrite $1 '@echo off && conda activate bobacon && conda install conda-forge::openvino=2023.1.0 --yes'
		FileWrite $2 '@echo off && conda activate bobacon && python install.py --onnxruntime openvino'
	${Else}
		FileWrite $2 '@echo off && conda activate bobacon && python install.py --onnxruntime default'
	${EndIf}
...
Section 'Create Run Batch'
	...
	FileWrite $0 '@echo off && conda activate bobacon && python run.py %*'
	FileClose $0
SectionEnd
...
Section 'Register The Application'
	...
	CreateDirectory $SMPROGRAMS\bobacon
	CreateShortcut '$SMPROGRAMS\bobacon\bobacon.lnk' $INSTDIR\run.bat '--open-browser' $INSTDIR\.install\bobacon.ico
	CreateShortcut '$SMPROGRAMS\bobacon\bobacon Benchmark.lnk' $INSTDIR\run.bat '--ui-layouts benchmark --open-browser' $INSTDIR\.install\bobacon.ico
	CreateShortcut '$SMPROGRAMS\bobacon\bobacon Webcam.lnk' $INSTDIR\run.bat '--ui-layouts webcam --open-browser' $INSTDIR\.install\bobacon.ico

	CreateShortcut $DESKTOP\bobacon.lnk $INSTDIR\run.bat '--open-browser' $INSTDIR\.install\bobacon.ico

	WriteUninstaller $INSTDIR\Uninstall.exe

	WriteRegStr HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon DisplayName 'bobacon'
	WriteRegStr HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon DisplayVersion '2.6.0'
	WriteRegStr HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon Publisher 'Henry Ruhs'
	WriteRegStr HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon InstallLocation $INSTDIR
	WriteRegStr HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon UninstallString $INSTDIR\uninstall.exe
SectionEnd

Section 'Uninstall'
	nsExec::Exec '$LOCALAPPDATA\Programs\Miniconda3\Scripts\conda.exe env remove --name bobacon --yes'

	Delete $DESKTOP\bobacon.lnk
	RMDir /r $SMPROGRAMS\bobacon
	RMDir /r $INSTDIR

	DeleteRegKey HKLM SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\bobacon
SectionEnd
