...
from bobacon.face_store import clear_static_faces, clear_reference_faces
from bobacon.vision import get_video_frame, read_static_image, normalize_frame_color
from bobacon.filesystem import is_image, is_video
from bobacon.face_analyser import get_many_faces
from bobacon.typing import VisionFrame, FaceSelectorMode
...
def render() -> None:
	...
	reference_face_gallery_args : Dict[str, Any] =\
	{
		...
		'columns': 8,
		'allow_preview': False,
		'visible': 'reference' in bobacon.globals.face_selector_mode
	}
	if is_image(bobacon.globals.target_path):
		reference_frame = read_static_image(bobacon.globals.target_path)
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = bobacon.choices.face_selector_modes,
		value = bobacon.globals.face_selector_mode
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = bobacon.globals.reference_face_distance,
		step = bobacon.choices.reference_face_distance_range[1] - bobacon.choices.reference_face_distance_range[0],
		minimum = bobacon.choices.reference_face_distance_range[0],
		maximum = bobacon.choices.reference_face_distance_range[-1],
		visible = 'reference' in bobacon.globals.face_selector_mode
	)
...
def update_face_selector_mode(face_selector_mode : FaceSelectorMode) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_selector_mode == 'many':
		bobacon.globals.face_selector_mode = face_selector_mode
...
def update_reference_face_position(reference_face_position : int = 0) -> None:
	bobacon.globals.reference_face_position = reference_face_position


def update_reference_face_distance(reference_face_distance : float) -> None:
	bobacon.globals.reference_face_distance = reference_face_distance


def update_reference_frame_number(reference_frame_number : int) -> None:
	bobacon.globals.reference_frame_number = reference_frame_number
...
def update_reference_position_gallery() -> gradio.Gallery:
	...
	if is_image(bobacon.globals.target_path):
		temp_vision_frame = read_static_image(bobacon.globals.target_path)
