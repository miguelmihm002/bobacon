...
from bobacon.audio import create_empty_audio_frame
from bobacon.common_helper import is_windows
from bobacon.content_analyser import analyse_stream
from bobacon.filesystem import filter_image_paths
from bobacon.typing import VisionFrame, Face, Fps
from bobacon.face_analyser import get_average_face
from bobacon.processors.frame.core import get_frame_processors_modules, load_frame_processor_module
from bobacon.ffmpeg import open_ffmpeg
from bobacon.vision import normalize_frame_color, read_static_images, unpack_resolution
from bobacon.uis.typing import StreamMode, WebcamMode
from bobacon.uis.core import get_ui_component, get_ui_components
...
def start(webcam_mode : WebcamMode, webcam_resolution : str, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	bobacon.globals.face_selector_mode = 'one'
	bobacon.globals.face_analyser_order = 'large-small'
...
def multi_process_capture(source_face : Face, webcam_capture : cv2.VideoCapture, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	with tqdm(desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = bobacon.globals.log_level in [ 'warn', 'error' ]) as progress:
		with ThreadPoolExecutor(max_workers = bobacon.globals.execution_thread_count) as executor:
			futures = []
