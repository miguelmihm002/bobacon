...
from bobacon.typing import OutputVideoEncoder, OutputVideoPreset, Fps
from bobacon.filesystem import is_image, is_video
from bobacon.uis.core import get_ui_components, register_ui_component
from bobacon.vision import detect_image_resolution, create_image_resolutions, detect_video_fps, detect_video_resolution, create_video_resolutions, pack_resolution
...
def render() -> None:
	...
	if is_image(bobacon.globals.target_path):
		output_image_resolution = detect_image_resolution(bobacon.globals.target_path)
	...
	bobacon.globals.output_path = bobacon.globals.output_path or '.'
	OUTPUT_PATH_TEXTBOX = gradio.Textbox(
		label = wording.get('uis.output_path_textbox'),
		value = bobacon.globals.output_path,
		max_lines = 1
	)
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_image_quality_slider'),
		value = bobacon.globals.output_image_quality,
		step = bobacon.choices.output_image_quality_range[1] - bobacon.choices.output_image_quality_range[0],
		minimum = bobacon.choices.output_image_quality_range[0],
		maximum = bobacon.choices.output_image_quality_range[-1],
		visible = is_image(bobacon.globals.target_path)
	)
	OUTPUT_IMAGE_RESOLUTION_DROPDOWN = gradio.Dropdown(
		...
		value = bobacon.globals.output_image_resolution,
		visible = is_image(bobacon.globals.target_path)
	)
	OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
		label = wording.get('uis.output_video_encoder_dropdown'),
		choices = bobacon.choices.output_video_encoders,
		value = bobacon.globals.output_video_encoder,
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		choices = bobacon.choices.output_video_presets,
		value = bobacon.globals.output_video_preset,
		visible = is_video(bobacon.globals.target_path)
	)
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_quality_slider'),
		value = bobacon.globals.output_video_quality,
		step = bobacon.choices.output_video_quality_range[1] - bobacon.choices.output_video_quality_range[0],
		minimum = bobacon.choices.output_video_quality_range[0],
		maximum = bobacon.choices.output_video_quality_range[-1],
		visible = is_video(bobacon.globals.target_path)
	)
	OUTPUT_VIDEO_RESOLUTION_DROPDOWN = gradio.Dropdown(
		...
		value = bobacon.globals.output_video_resolution,
		visible = is_video(bobacon.globals.target_path)
	)
	OUTPUT_VIDEO_FPS_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_fps_slider'),
		value = bobacon.globals.output_video_fps,
		step = 0.01,
		minimum = 1,
		maximum = 60,
		visible = is_video(bobacon.globals.target_path)
