...
from bobacon.common_helper import calc_float_step, calc_int_step
from bobacon.typing import FaceMaskRegion, FaceMaskType
...
def render() -> None:
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calc_float_step(bobacon.choices.face_mask_blur_range),
	...
	with gradio.Group():
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calc_int_step(bobacon.choices.face_mask_padding_range),
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider]:
	face_mask_types = face_mask_types or bobacon.choices.face_mask_types
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or bobacon.choices.face_mask_regions
