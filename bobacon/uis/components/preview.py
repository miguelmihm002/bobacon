...
from bobacon import logger, wording
from bobacon.audio import get_audio_frame, create_empty_audio_frame
from bobacon.common_helper import get_first
from bobacon.core import conditional_append_reference_faces
from bobacon.face_analyser import get_average_face, clear_face_analyser
from bobacon.face_store import clear_static_faces, get_reference_faces, clear_reference_faces
from bobacon.typing import Face, FaceSet, AudioFrame, VisionFrame
from bobacon.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_resolution, read_static_image, read_static_images
from bobacon.filesystem import is_image, is_video, filter_audio_paths
from bobacon.content_analyser import analyse_frame
from bobacon.processors.frame.core import load_frame_processor_module
from bobacon.uis.core import get_ui_component, get_ui_components, register_ui_component
...
def render() -> None:
	...
	if source_audio_path and bobacon.globals.output_video_fps and bobacon.globals.reference_frame_number:
		temp_audio_frame = get_audio_frame(source_audio_path, bobacon.globals.output_video_fps, bobacon.globals.reference_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(bobacon.globals.target_path):
		target_vision_frame = read_static_image(bobacon.globals.target_path)
	...
	if is_video(bobacon.globals.target_path):
		temp_vision_frame = get_video_frame(bobacon.globals.target_path, bobacon.globals.reference_frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_image_args['value'] = normalize_frame_color(preview_vision_frame)
		preview_image_args['visible'] = True
		preview_frame_slider_args['value'] = bobacon.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(bobacon.globals.target_path)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
	for frame_processor in bobacon.globals.frame_processors:
	...
	source_audio_frame = create_empty_audio_frame()
	if source_audio_path and bobacon.globals.output_video_fps and bobacon.globals.reference_frame_number:
		reference_audio_frame_number = bobacon.globals.reference_frame_number
		if bobacon.globals.trim_frame_start:
			reference_audio_frame_number -= bobacon.globals.trim_frame_start
		temp_audio_frame = get_audio_frame(source_audio_path, bobacon.globals.output_video_fps, reference_audio_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(bobacon.globals.target_path):
		target_vision_frame = read_static_image(bobacon.globals.target_path)
	...
	if is_video(bobacon.globals.target_path):
		temp_vision_frame = get_video_frame(bobacon.globals.target_path, frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_vision_frame = normalize_frame_color(preview_vision_frame)
		return gradio.Image(value = preview_vision_frame)
