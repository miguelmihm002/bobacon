...
from bobacon.uis.typing import File
from bobacon.filesystem import get_file_size, is_image, is_video
from bobacon.uis.core import register_ui_component
from bobacon.vision import get_video_frame, normalize_frame_color
...
def render() -> None:
	...
	is_target_image = is_image(bobacon.globals.target_path)
	is_target_video = is_video(bobacon.globals.target_path)
	TARGET_FILE = gradio.File(
		...
		value = bobacon.globals.target_path if is_target_image or is_target_video else None
	)
	target_image_args =\
	{
		'show_label': False,
	...
	if is_target_video:
		if get_file_size(bobacon.globals.target_path) > FILE_SIZE_LIMIT:
			preview_vision_frame = normalize_frame_color(get_video_frame(bobacon.globals.target_path))
...
def update(file : File) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		bobacon.globals.target_path = file.name
	...
	bobacon.globals.target_path = None
