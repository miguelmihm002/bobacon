...
from bobacon.app_context import detect_app_context
from bobacon.execution import create_execution_providers, has_execution_provider
from bobacon.thread_helper import thread_lock
from bobacon.typing import DownloadSet, ExecutionProviderKey, InferencePool, InferencePoolSet, ModelInitializer
...
def resolve_execution_provider_keys(model_context : str) -> List[ExecutionProviderKey]:
	if has_execution_provider('coreml') and (model_context.startswith('bobacon.processors.modules.age_modifier') or model_context.startswith('bobacon.processors.modules.frame_colorizer')):
		return [ 'cpu' ]
