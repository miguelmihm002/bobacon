...
from bobacon.typing import ProcessFrames, QueuePayload
from bobacon.execution import encode_execution_providers
...
def load_frame_processor_module(frame_processor : str) -> Any:
	try:
		frame_processor_module = importlib.import_module('bobacon.processors.frame.modules.' + frame_processor)
...
def multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : ProcessFrames) -> None:
	...
	with tqdm(total = len(queue_payloads), desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = bobacon.globals.log_level in [ 'warn', 'error' ]) as progress:
		progress.set_postfix(
		{
			'execution_providers': encode_execution_providers(bobacon.globals.execution_providers),
			'execution_thread_count': bobacon.globals.execution_thread_count,
			'execution_queue_count': bobacon.globals.execution_queue_count
		})
		with ThreadPoolExecutor(max_workers = bobacon.globals.execution_thread_count) as executor:
			...
			queue_per_future = max(len(queue_payloads) // bobacon.globals.execution_thread_count * bobacon.globals.execution_queue_count, 1)
