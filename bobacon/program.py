...
from bobacon import config, metadata, state_manager, wording
from bobacon.common_helper import create_float_metavar, create_int_metavar
from bobacon.execution import get_execution_provider_choices
from bobacon.filesystem import list_directory
from bobacon.jobs import job_store
from bobacon.processors.core import get_processors_modules
from bobacon.program_helper import remove_args, suggest_face_detector_choices
...
def create_config_program() -> ArgumentParser:
	...
	group_paths.add_argument('-c', '--config-path', help = wording.get('help.config_path'), default = 'bobacon.ini')
...
def create_face_detector_program() -> ArgumentParser:
	...
	group_face_detector.add_argument('--face-detector-model', help = wording.get('help.face_detector_model'), default = config.get_str_value('face_detector.face_detector_model', 'yoloface'), choices = bobacon.choices.face_detector_set.keys())
	...
	group_face_detector.add_argument('--face-detector-angles', help = wording.get('help.face_detector_angles'), type = int, default = config.get_int_list('face_detector.face_detector_angles', '0'), choices = bobacon.choices.face_detector_angles, nargs = '+', metavar = 'FACE_DETECTOR_ANGLES')
	group_face_detector.add_argument('--face-detector-score', help = wording.get('help.face_detector_score'), type = float, default = config.get_float_value('face_detector.face_detector_score', '0.5'), choices = bobacon.choices.face_detector_score_range, metavar = create_float_metavar(bobacon.choices.face_detector_score_range))
...
def create_face_landmarker_program() -> ArgumentParser:
	...
	group_face_landmarker.add_argument('--face-landmarker-model', help = wording.get('help.face_landmarker_model'), default = config.get_str_value('face_landmarker.face_landmarker_model', '2dfan4'), choices = bobacon.choices.face_landmarker_models)
	group_face_landmarker.add_argument('--face-landmarker-score', help = wording.get('help.face_landmarker_score'), type = float, default = config.get_float_value('face_landmarker.face_landmarker_score', '0.5'), choices = bobacon.choices.face_landmarker_score_range, metavar = create_float_metavar(bobacon.choices.face_landmarker_score_range))
...
def create_face_selector_program() -> ArgumentParser:
	...
	group_face_selector.add_argument('--face-selector-order', help = wording.get('help.face_selector_order'), default = config.get_str_value('face_selector.face_selector_order', 'large-small'), choices = bobacon.choices.face_selector_orders)
	group_face_selector.add_argument('--face-selector-age-start', help = wording.get('help.face_selector_age_start'), type = int, default = config.get_int_value('face_selector.face_selector_age_start'), choices = bobacon.choices.face_selector_age_range, metavar = create_int_metavar(bobacon.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-age-end', help = wording.get('help.face_selector_age_end'), type = int, default = config.get_int_value('face_selector.face_selector_age_end'), choices = bobacon.choices.face_selector_age_range, metavar = create_int_metavar(bobacon.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-gender', help = wording.get('help.face_selector_gender'), default = config.get_str_value('face_selector.face_selector_gender'), choices = bobacon.choices.face_selector_genders)
	group_face_selector.add_argument('--face-selector-race', help = wording.get('help.face_selector_race'), default = config.get_str_value('face_selector.face_selector_race'), choices = bobacon.choices.face_selector_races)
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-mask-types', help = wording.get('help.face_mask_types').format(choices = ', '.join(bobacon.choices.face_mask_types)), default = config.get_str_list('face_masker.face_mask_types', 'box'), choices = bobacon.choices.face_mask_types, nargs = '+', metavar = 'FACE_MASK_TYPES')
	group_face_masker.add_argument('--face-mask-blur', help = wording.get('help.face_mask_blur'), type = float, default = config.get_float_value('face_masker.face_mask_blur', '0.3'), choices = bobacon.choices.face_mask_blur_range, metavar = create_float_metavar(bobacon.choices.face_mask_blur_range))
	...
	group_face_masker.add_argument('--face-mask-regions', help = wording.get('help.face_mask_regions').format(choices = ', '.join(bobacon.choices.face_mask_regions)), default = config.get_str_list('face_masker.face_mask_regions', ' '.join(bobacon.choices.face_mask_regions)), choices = bobacon.choices.face_mask_regions, nargs = '+', metavar = 'FACE_MASK_REGIONS')
...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--output-image-quality', help = wording.get('help.output_image_quality'), type = int, default = config.get_int_value('output_creation.output_image_quality', '80'), choices = bobacon.choices.output_image_quality_range, metavar = create_int_metavar(bobacon.choices.output_image_quality_range))
	...
	group_output_creation.add_argument('--output-audio-encoder', help = wording.get('help.output_audio_encoder'), default = config.get_str_value('output_creation.output_audio_encoder', 'aac'), choices = bobacon.choices.output_audio_encoders)
	...
	group_output_creation.add_argument('--output-video-quality', help = wording.get('help.output_video_quality'), type = int, default = config.get_int_value('output_creation.output_video_quality', '80'), choices = bobacon.choices.output_video_quality_range, metavar = create_int_metavar(bobacon.choices.output_video_quality_range))
...
def create_processors_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_processors = list_directory('bobacon/processors/modules')
...
def create_uis_program() -> ArgumentParser:
	...
	group_uis.add_argument('--ui-workflow', help = wording.get('help.ui_workflow'), default = config.get_str_value('uis.ui_workflow', 'instant_runner'), choices = bobacon.choices.ui_workflows)
	return program
...
def create_execution_program() -> ArgumentParser:
	...
	group_execution.add_argument('--execution-thread-count', help = wording.get('help.execution_thread_count'), type = int, default = config.get_int_value('execution.execution_thread_count', '4'), choices = bobacon.choices.execution_thread_count_range, metavar = create_int_metavar(bobacon.choices.execution_thread_count_range))
	group_execution.add_argument('--execution-queue-count', help = wording.get('help.execution_queue_count'), type = int, default = config.get_int_value('execution.execution_queue_count', '1'), choices = bobacon.choices.execution_queue_count_range, metavar = create_int_metavar(bobacon.choices.execution_queue_count_range))
...
def create_memory_program() -> ArgumentParser:
	...
	group_memory.add_argument('--system-memory-limit', help = wording.get('help.system_memory_limit'), type = int, default = config.get_int_value('memory.system_memory_limit', '0'), choices = bobacon.choices.system_memory_limit_range, metavar = create_int_metavar(bobacon.choices.system_memory_limit_range))
...
def create_log_level_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_misc = program.add_argument_group('misc')
	group_misc.add_argument('--log-level', help = wording.get('help.log_level'), default = config.get_str_value('misc.log_level', 'info'), choices = bobacon.choices.log_level_set.keys())
...
def create_job_status_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	program.add_argument('job_status', help = wording.get('help.job_status'), choices = bobacon.choices.job_statuses)
	return program
